...
export default function MemberListItems() {
  ...
  // console.log(state);
  // console.log(initialStateMember)
  ...
  return (
    <>
      ...
      {members.map((member: any) => (
        <div key={member.id} className="flex justify-between items-center p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
          <div className="flex flex-col">

            <h5 className="mb-2 text-xl font-medium tracking-tight text-gray-900 dark:text-white">{member.name}</h5>
            <h5 className="mb-2 text-l  tracking-tight text-gray-800 dark:text-white">{member.email}</h5>
          </div>
          <div className="flex">
            <button onClick={() => deleteMember(dispatchMembers, { id: member.id })} className="hover:text-red-600">
